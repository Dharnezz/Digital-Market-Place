...
- **`DataSourceUrlConfig`**: when `DATABASE_URL` is set, platform-provided `postgres://...` / `postgresql://...` connection strings are parsed with `java.net.URI` and converted to a credential-free `jdbc:postgresql://host:port/db` URL; username/password are extracted from the URI userinfo and supplied as separate DataSource properties (used when `DB_USERNAME`/`DB_PASSWORD` are unset). Local development (no `DATABASE_URL`) is unchanged.
...
### Tests
- Backend suite now **155 tests** (up from 138): guards for illegal product-status transitions covered by unit tests.
